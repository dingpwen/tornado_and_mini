...
function getDataPoints(data, minRange, maxRange, xAxisPoints, eachSpacing, opts, config) {
    ...
    data.forEach(function (item, index) {
      ...
      if (item !== null) {
        if (index == 0) {
          validpoints.push(index);
        } else if (index + 1 == data.length) {
          if (data[validpoints[validpoints.length - 1]] != 0 && validpoints[validpoints.length - 1] + 1 != index) {
            validpoints.push(validpoints[validpoints.length - 1] + 1);
          }
          validpoints.push(index);
        } else if (item != 0) {
          if (data[validpoints[validpoints.length - 1]] == 0 && index > 1) {
            validpoints.push(index - 1);
          }
          validpoints.push(index);
        }
      }
      // if (item !== null && (index == 0 || index + 1 == data.length || item != 0)) {
      //     validpoints.push(index);
      //   }
    });
    ...
    data.forEach(function (item, index) {
        if (item === null) {
            points.push(null);
        // } else if (validindex < validpoints.length &&
        //             (validpoints[validindex] == index ||
        //               (validpoints.length > 2 &&
        //                 ((validindex == 1 && validpoints[validindex] - 1 == index) ||
        //                 (validindex + 1 == validpoints.length && validpoints[validindex - 1] +1 == index))))) {
        //     var point = {};
        ...
        //     points.push(point);
        //     if (validpoints[validindex] == index) {
        //       validindex++;
        //     }
        // }
        } else if (validpoints[validindex] == index) {
            validindex++;
            var point = {};
            point.x = xAxisPoints[index] + Math.round(eachSpacing / 2);
            var height = validHeight * (item - minRange) / (maxRange - minRange);
...
function drawPointText(points, series, config, context) {
    ...
    points.forEach(function (item, index) {
        if (item !== null) {
            var formatVal = series.format ? series.format(data[validpoints[index]]) : data[validpoints[index]];
...
function drawLineDataPoints(series, opts, config, context) {
    ...
    var _getXAxisPoints3 = getXAxisPoints(opts.categories, opts, config),
        ...
        xStart = _getXAxisPoints3.startX,
    ...
    series.forEach(function (eachSeries, seriesIndex) {
        ...
        calPoints.push(points);
        var validHeight = opts.height - 2 * config.padding - config.xAxisHeight - config.legendHeight;
        var height = validHeight * (0 - minRange) / (maxRange - minRange);
        var yStart = opts.height - config.xAxisHeight - config.legendHeight - Math.round(height) - config.padding;
        var splitPointList = splitPoints(points);
        ...
        splitPointList.forEach(function (points, index) {
            ...
            if (points.length === 1) {
                ...
                context.stroke();
                context.lineTo(points[points.length - 1].x, yStart);
                context.lineTo(points[0].x, yStart);
            }
            context.closePath();
            var grd = context.createLinearGradient(0, yMin, 0, validHeight);
            grd.addColorStop(0, 'rgba(49, 191, 191, 0.6)');
            grd.addColorStop(1, 'rgba(49, 191, 191, 0)');
        ...
        if (opts.dataPointShape !== false) {
            ...
            points.forEach(function (item, index) {
              if (item !== null) {
                //context.beginPath();
                context.drawImage("../../images/cig_point.png", item.x - 8, item.y - 8, 16, 16)
                //context.draw()
                //context.closePath();
                // context.beginPath();
                // context.moveTo(item.x + 3.5, item.y);
                // context.setFillStyle("#31BFBF");
                // context.arc(item.x, item.y, 4, 0, 2 * Math.PI, false);
                // context.fill();
                // context.closePath();
                // context.beginPath();
...
function drawXAxis(categories, opts, config, context) {
    ...
    var eachSpacing1 = Math.floor(spacingValid / config.yAxisSplit);
    //var yAxisTotalWidth = config.yAxisWidth + config.yAxisTitleWidth;
  var endY = opts.height - config.padding - config.xAxisHeight;//config.padding + eachSpacing1 * config.yAxisSplit + 2;
    ...
    if (opts.xAxis.disableGrid !== true) {
        if (opts.xAxis.type === 'calibration') {
            xAxisPoints.forEach(function (item, index) {
                if (index < xAxisPoints.length - 1 ) {
                  context.setLineDash([1, 5]);
                  context.moveTo(item + Math.round(eachSpacing / 2), 0);
                  context.lineTo(item + Math.round(eachSpacing / 2), endY);
                }
            });
        } else {
            xAxisPoints.forEach(function (item, index) {
    ...
    categories = categories.map(function (item, index) {
      if (opts.month) {
        if (index == 0 || (index + 1) % ratio == 0 || (index + 1) == categories.length) {
          return item
        } else {
          return ''
        }
      } else {
        return (index % ratio !== 0 ? '' : item) || (index + 1 == categories.length ? item : '');
      }
    });
    ...
    if (config._xAxisTextAngle_ === 0) {
      ...
      categories.forEach(function (item, index) {
        ...
        if (Array.isArray(item)) {
          var offset = eachSpacing / 2 - textWidth / 2;
          for (var b = 0; b < item.length; b++) {
            context.fillText(item[b], xAxisPoints[index] + offset + 15, startY + config.fontSize + 5 + 15 * b);
          }
        } else {
          ...
          context.fillText(item, xAxisPoints[index] + offset + 15, startY + config.fontSize + 5);
        }
        context.closePath();
        context.stroke();
        context.restore();
